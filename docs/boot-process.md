...
| **Userspace location** | `rootfs/linux/{arch}/` (built by xtask from busybox source)              | `rootfs/zircon/{arch}/` (petal flat binaries)        | Decouple from kernel build (#175), unified rootfs (#176)                  |
| **Binary format**      | Standard ELF (static, musl-linked)                                       | Flat binary (objcopy from ELF)                       | Zircon should load ELF directly (#177)                                    |
| **Rootfs/bootfs**      | SFS image (bare-metal) / HostFS dir (libos)                              | SFS image (bare-metal) / HostFS dir (libos), ZBI fallback | Both can load at runtime (#175 partially done)                       |
...
| **LibOS rootfs**       | HostFS from `rootfs/linux/{host_arch}/`                                  | HostFS from `rootfs/zircon/{host_arch}/`, ZBI fallback | Unified (#178)                                                         |
